...
const Pagination = ({
  ...
  return (
    <div
      className={`flex flex-col md:flex-row md:items-center justify-between gap-2 mt-6 text-[#6C6C6C] h-auto 
        ${fullWidth ? "w-full" : "w-1/2"}`}
    >
   
      <div className="hidden md:flex flex-col md:flex-row md:items-center justify-between w-full gap-2">
        {/* Showing entries */}
        <p className="text-sm h-10 flex items-center">
          Showing {(page - 1) * perPage + 1} to{" "}
          {Math.min(page * perPage, totalItems)} of {totalItems} entries
